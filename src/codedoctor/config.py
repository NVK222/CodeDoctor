...
class Config:
    def __init__(
        ...
        self._root_dir = self._get_config_root()
        self.search_dir = self.root_dir
        ...
        if root_dir:
            ...
            self.test_dir = self.root_dir / "tests"
        if search_dir:
            self.search_dir = self.root_dir / Path(search_dir)
        if test_dir:
            self.test_dir = self.root_dir / Path(test_dir)
    ...
    def root_dir(self, value: str | Path) -> None:
        ...
        self.test_dir = self._root_dir / "tests"
